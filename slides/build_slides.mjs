...
text(item, 'AI-ENG-110 / Topic 4 / 17 September 2026', 80, 523, 1120, 50, 24);
...
item = slide('Docker and CI',
  'Owner: Bailar. Sources: Dockerfile, .dockerignore, .github/workflows/ci.yml and report/report.md. Both image stages use python:3.12.6-slim. Appuser owns /app for SQLite and output files. CI smoke commands run with --network none. Docker was unavailable locally, so build/run is pending. Global lint and mypy failures in other owners/frozen files remain. Branch protection must require CI checks.');
...
text(item, 'The runtime uses a non-root account with writable outputs.\nCI checks lint, types, coverage and the Docker build.\nContainer smoke tests disable networking.',
  80, 290, 1120, 165, 30);
text(item, 'Verification still needed', 80, 502, 1120, 50, 30, '#153B57', true);
text(item, 'Docker build/run and team fixes for existing lint/type errors',
  80, 559, 1120, 90, 28);

item = slide('Questions and discussion',
  'Sources: report/report.md and supplied ownership rules. Suggested answers: the CLI null cache prevents persistent storage construction under --no-cache; fake Researcher isolates the CLI contract; the team release still needs Docker verification, other owners lint/type fixes and report/slide submissions. AI disclosure: Codex assisted Bailar-owned code, documentation and these slides.');
